// ChromaCorrect Content Script - Injected into Web Pages
(function() {
  let activeSettings = {"isEnabled":true,"deficiency":"deuteranopia","assistMode":"daltonize_patterns","intensity":85,"safePalette":"okabeIto","enableColorInspector":true,"enableDirectLabels":true,"enableEdgeStroke":true,"enableImageCorrection":true,"imageFilterScope":"all_images","strokeWidth":2,"patternScale":1};
  let tooltipEl = null;

  // 1. Inject SVG Filters & Patterns into Document Head
  function injectSvgAssets() {
    if (document.getElementById('chromacorrect-svg-defs')) return;

    const svg = document.createElementNS('http://www.w3.org/2000/svg', 'svg');
    svg.id = 'chromacorrect-svg-defs';
  ...
  // 2. Apply styling and filters to chart elements and web images
  function applyFilters(settings) {
    activeSettings = settings;
    injectSvgAssets();

    // Selectors for charts, visualizations, diagrams, infographics, and web images
    const shouldIncludeImages = settings.enableImageCorrection !== false;
    const baseChartSelectors = 'svg:not(#chromacorrect-svg-defs):not(#chromagraph-svg-defs), canvas, [role="img"], .recharts-wrapper, .highcharts-container, .plotly, picture';
    const imageSelectors = shouldIncludeImages ? ', img, [style*="background-image"]' : ', img[src*="chart"], img[src*="graph"], img[src*="diagram"], img[src*="map"]';
    const targetSelectors = baseChartSelectors + imageSelectors;
    const targetElements = document.querySelectorAll(targetSelectors);

    targetElements.forEach((el) => {
      if (el.id === 'chromacorrect-svg-defs' || el.closest('#chromacorrect-svg-defs') || el.id === 'chromagraph-svg-defs' || el.closest('#chromagraph-svg-defs')) return;

      if (!settings.isEnabled) {
        el.style.filter = '';
        el.classList.remove('chromacorrect-active', 'chromagraph-active');
        return;
      }

      el.classList.add('chromacorrect-active');

      let filterVal = '';
      if (settings.assistMode === 'daltonize' || settings.assistMode === 'daltonize_patterns') {
        const intensityBoost = 1 + ((settings.intensity || 85) / 100) * 0.25;
        if (settings.deficiency && settings.deficiency.includes('deuter')) {
          filterVal = `url(#cg-daltonize-deuteranopia) contrast(${intensityBoost.toFixed(2)})`;
        } else if (settings.deficiency && settings.deficiency.includes('protan')) {
          filterVal = `url(#cg-daltonize-protanopia) contrast(${intensityBoost.toFixed(2)})`;
        } else if (settings.deficiency && settings.deficiency.includes('tritan')) {
          filterVal = `url(#cg-daltonize-tritanopia) contrast(${(intensityBoost * 0.95).toFixed(2)})`;
        } else if (settings.deficiency === 'achromatopsia') {
          filterVal = `contrast(${(intensityBoost * 1.2).toFixed(2)}) saturate(0)`;
        }
  ...
  function injectSvgPatterns() {
    const shapes = document.querySelectorAll('svg:not(#chromacorrect-svg-defs):not(#chromagraph-svg-defs) path, svg:not(#chromacorrect-svg-defs):not(#chromagraph-svg-defs) rect');
  ...
  function initColorInspector() {
    ...
    tooltipEl.id = 'chromacorrect-inspector-tooltip';
    ...
    document.addEventListener('mousemove', (e) => {
      ...
      if (!target || target.id === 'chromacorrect-inspector-tooltip' || target.id === 'chromagraph-inspector-tooltip' || !target.closest('svg, canvas, [role="img"]')) {
        tooltipEl.style.display = 'none';
        return;
      }
  ...
  if (window.chrome && chrome.runtime && chrome.runtime.onMessage) {
    chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
      if (request.type === 'CHROMACORRECT_UPDATE' || request.type === 'CHROMAGRAPH_UPDATE') {
  ...
  // Dynamic Image & DOM Mutation Observer (detect newly loaded images, infinite scrolling, SPAs)
  let mutationTimeout = null;
  const observer = new MutationObserver(() => {
    if (mutationTimeout) clearTimeout(mutationTimeout);
    mutationTimeout = setTimeout(() => {
      if (activeSettings && activeSettings.isEnabled) {
        applyFilters(activeSettings);
      }
    }, 250);
  });
